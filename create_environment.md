...
- [Installing Pixi and uv](#installing-pixi-and-uv)
   ...
   * [uv installation](#uv-installation)
      ...
      + [Linux](#linux-1)
- [Manual Pixi commands](#manual-pixi-commands)
   * [PowerShell](#powershell)
   * [cmd.exe](#cmdexe)
   * [Git Bash](#git-bash)
- [Manual uv commands](#manual-uv-commands)
...
This architecture-based storage assumes [Pixi detached environments](#pixi-detached-environments), which store the actual environment files separately from the project definition.
...
Pixi's `detached-environments` setting controls where workspace environments are stored instead of the default `.pixi/envs` folder inside the workspace. Pixi supports this in configuration, including per-project `.pixi/config.toml`. citeturn0search16
...
Pixi's official installation docs recommend the standalone installer, and note that rerunning the installer updates Pixi, or you can use `pixi self-update`. citeturn0search0turn0search16
...
Astral's official docs recommend the standalone installer on both Windows and Linux. citeturn0search1turn0search3turn0search7
...
uv can also install Python on demand, and it can install specific Python versions with `uv python install 3.12`. citeturn0search11

---

# Manual Pixi commands

These are the raw commands if you want to call the setup scripts directly instead of using the shell wrapper functions.
